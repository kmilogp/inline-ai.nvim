...
tests['prompt_window.open builds prompt and closes'] = function()
  local vim_mock = make_vim_mock()
  local prompt_cb = nil
  ...
  vim_mock.api.nvim_create_buf = function()
    return 10
  end
  vim_mock.api.nvim_open_win = function()
    return 20
  end
  vim_mock.api.nvim_win_is_valid = function()
    return true
  end
  vim_mock.api.nvim_win_close = function()
    closed = closed + 1
  end
  vim_mock.api.nvim_buf_is_valid = function()
    return true
  end
  vim_mock.api.nvim_buf_delete = function()
    deleted = deleted + 1
  end
  vim_mock.fn.prompt_setcallback = function(_, cb)
    prompt_cb = cb
  end
  vim_mock.fn.prompt_setinterrupt = function(_, cb)
    interrupt_cb = cb
  end

  with_vim(vim_mock, function()
    local prompt_window = reload('opencode.prompt_window')
    local called = {}
    prompt_window.open({
      profile_name = 'fast',
      ...
      resolve_profile = function()
        return 'fast', { model = 'qwen', provider = 'ollama', include_full_file_context = true, auto_apply = true }, { transport = 'ollama_http', auto_apply = true }, nil
      end,
      get_context = function()
        return { file = 'x.lua' }
      end,
      build_prompt = function(ctx)
        return 'PROMPT:' .. ctx.input
      end,
    })

    assert_true(type(prompt_cb) == 'function')
    prompt_cb('hello')
    assert_eq(called.prompt, 'PROMPT:hello')
    assert_eq(called.model, 'qwen')
    assert_eq(called.name, 'fast')
    assert_eq(called.provider_name, 'ollama')
    assert_eq(closed, 1)
    assert_eq(deleted, 1)
