...
local function load_sessions()
  if not M.config.persist_sessions then return {} end
...
local function save_sessions()
  ...
  if not ok_encode then return end
  pcall(vim.fn.writefile, { encoded }, M.config.sessions_file)
end

local function ensure_sessions()
  if M._sessions == nil then M._sessions = load_sessions() end
end
...
function M.send_prompt(prompt, model, name)
  ...
  run_client(vim.fn.json_encode(payload), function(ok, data, err)
    ...
    if not ok then
      vim.notify('Opencode failed: ' .. err, vim.log.levels.ERROR, { title = title })
      return
    end

    if data.sessionId then M.set_session_id(data.sessionId) end

    local text = data.text or ''
    if text == '' then
      vim.notify('Opencode returned no text', vim.log.levels.WARN, { title = title })
      return
    end
